...
- **Cocos Creator 3.x** — `assets/` plus a `package.json` declaring `creator.version`. Primary line: `3.8`.
- **Cocos Creator 2.x** — `assets/` plus a root `project.json` declaring a `2.x` version (the shape `dev-cocos-port-2x` produces). Editor-independent setup applies; the editor MCP extension does not exist for this line.
...
The engine decides four things and nothing else: which preflight runs, which ignore template applies, which editor extensions are installable, and which live phase the checkpoint routes to. Inside Cocos, the **major line** decides two more: which build of the vendored editor extensions is installed and where (`extensions/` on 3.x, `packages/` on 2.x), and whether the MCP extension exists at all — Creator 3.x gets it and a live phase, Creator 2.x gets neither. Everything else, including both editor extensions, applies to both lines. Beads, Serena, Better Context, CodeGraph, CocoIndex Code, the Blender MCP, and the video analyzer are engine-neutral and are configured the same way in both.

## Objective

Prepare the detected engine's repository for the copied agent packages — an exact Unity `6000.3.21f1` project, or a Cocos Creator project on either the `3.8` or the `2.4` line. Initialize Git when needed, establish ignore rules, and configure Beads, Serena, caption-gated video analysis, official Unity MCP from `com.unity.ai.assistant`, the official Blender MCP server, NuGetForUnity, ZLinq, VContainer, ProBuilder, VFX Graph, glTFast, Cinemachine, Burst, Collections, approved Unity asset packages and their registered update checks, Better Context Unity, CodeGraph, and optionally CocoIndex Code plus local Docling conversion only when the user explicitly provides a non-empty Voyage API key for the current setup run. Configure every client bundle this repository carries, so Codex and Claude Code end up with the same tools. In a Cocos project the engine-specific work is instead `setup-cocos-project-preflight`, `setup-cocos-gitignore`, `setup-cocos-extensions`, and — on Creator 3.x only — `setup-cocos-mcp`; the Unity package, ZLinq, and asset-import steps do not apply and must not be attempted. Do not install Unity Hub/Editor or a Cocos Creator editor, and do not upgrade an older project; for Unity, stop unless both project and installed Editor are exactly `6000.3.21f1`. Computer Use and all UI automation are prohibited for this role in every phase.
...
8b. `setup-game-toolchain`, only when the user wants the browser-game fetch/port skills usable (it installs `python3` and the optional `oxipng`)
9. `setup-cocos-extensions` (Cocos only), then `setup-unity-mcp` preparation pass (Unity only) or `setup-cocos-mcp` (Cocos **3.x** only)
...
3. Read and run the engine's ignore skill: `setup-unity-gitignore` (Unity, keeping NuGet restore declarations visible) or `setup-cocos-gitignore` (Cocos, keeping `*.meta`, `/settings/`, `/build-templates/`, `/packages/`, and `package-lock.json` visible — ignoring a `.meta` file destroys every scene reference silently). One Cocos template covers both Creator lines, but probe the line the preflight reported: `profiles/` and `extensions/*/node_modules/` on 3.x, `packages/*/node_modules/` on 2.x, and `.dev-tools-trash/` on both. In a 2.x project `/packages/` is authored source — it holds the project's own editor extensions — so a broad `packages/` ignore silently drops them from the repository. Verify effective engine, AI-tool, and Markdown ignores either way. `.claude/`, `.mcp.json`, `.ctx.json`, and `codegraph.json` are part of that ignored agent-local surface.
...
9b. Read and run `setup-game-toolchain` only when the user wants the browser-game fetch and Cocos port skills usable. It owns exactly two prerequisites — a real `python3` on PATH and the optional `oxipng` — and nothing else; Node.js stays with `setup-video-analyzer`. Report `deferred` when the user does not ask for it.
...
10'. Read and run `setup-cocos-extensions`. It installs the vendored project-local editor extensions — `dev-tools` (the quick-dev menu) and `minigame-pack` (the mini-game subpackage packer) — from this package's own `assets/`, with no network access. **Both engine lines are vendored**, so both get installed; the skill routes on the preflight's `creator_major` and on each vendored folder's `engine_line`/`install_dir`:
  - Creator 3.x → `dev-tools` and `minigame-pack` into `<project>/extensions/`.
  - Creator 2.x → `dev-tools-2x` and `minigame-pack-2x` into `<project>/packages/`, installed under their unprefixed names (`packages/dev-tools/`, `packages/minigame-pack/`) because Creator 2.x resolves a package by the `name` in its `package.json`.

  Never install one line's build into the other line's project, and never both at once. Never overwrite a locally modified extension: back it up, name the differing files, and ask. Each 2.x build ships a self-contained `test.js` — running `node <installed>/test.js` needs no editor and is the cheapest proof the copy is intact, including that every menu entry still has a handler.
10b'. **Creator 2.x stops after the extensions.** There is no editor MCP extension for that line and none is planned, so skip `setup-cocos-mcp`, skip the editor-open checkpoint, and go straight to step 13'. Everything else a 2.x project needs — ignore rules, Beads, Serena, Better Context, CodeGraph, CocoIndex, and now both editor extensions — is editor-independent. Report the extensions as installed and the MCP rows as `not-applicable (Creator 2.x)` rather than leaving them blank.
10c'. Read and run `setup-cocos-mcp` (Creator 3.x only): verify or install the `funplay-cocos-mcp` editor extension from its official sources, read the project's real host/port from `funplay-cocos-mcp.config.json` rather than assuming `8765`, and register `funplay_cocos` for every in-scope client with the same URL, the same approval gate on `execute_javascript`, `execute_scene_script`, and `execute_editor_script`, and the same denial of the desktop input-simulation and desktop-capture tools.
11'. (Creator 3.x) The MCP server is embedded in the Cocos Editor, so nothing live can be verified while it is closed. Save the checkpoint, ask the user to open the exact project in Cocos Creator and to restart each client whose configuration changed, and resume at the Cocos live phase. This is `pending user action`, not a failure.
12'. (Creator 3.x) Cocos live phase: confirm `GET /health` and `GET /tools` answer on the configured port, confirm each client lists `funplay_cocos`, make one read-only call (`get_project_info`), and confirm an execution tool prompts for approval rather than running silently.
...
- In a Cocos project: the preflight reported the `creator_version` and `creator_major`, and exactly that line's phases ran — a 2.x project installed no MCP extension, registered no `funplay_cocos` entry, and never waited for an editor, while still getting its ignore rules and both editor extensions; a 3.x project did all of those plus the MCP extension, its registration, and the live verification.
- `setup-cocos-extensions` reported one state per vendored extension (`dev-tools`, `minigame-pack`) with its engine line and install path, each verified against its `vendor-manifest.json` hashes after copying, with any locally modified file reported instead of overwritten, and an editor reload named as pending when one is required. Both lines install both extensions: a 3.x project has them under `extensions/`, a 2.x project under `packages/` with no `-2x` suffix on the installed folder, and neither project has the other line's build anywhere.
- In a Cocos 2.x project: `.gitignore` ignores `library/`, `temp/`, `build/`, `local/`, and `.dev-tools-trash/` while `*.meta`, `/settings/`, `/packages/`, and `package-lock.json` stay tracked — `packages/` holds the project's own editor extensions and is authored source, so only `packages/*/node_modules/` is ignored. The two 2.x extensions verify from their installed location with `node <installed>/test.js`, which needs no editor.
- In a Cocos 3.x project: the preflight reported a `3.8` `creator.version`; `.gitignore` ignores `library/`, `temp/`, `build/`, `local/`, and `profiles/` while `*.meta`, `/settings/`, `/build-templates/`, and `package-lock.json` stay tracked; the `funplay-cocos-mcp` extension version and install route are recorded; each in-scope client holds one `funplay_cocos` entry at the configured URL with the three execution tools approval-gated and the desktop input/capture tools denied; the editor was reachable for live verification or that verification is reported as pending; and Better Context's Cocos maps verify with the bundle contract and any unresolved component types listed explicitly.
...
- When the game fetch/port skills were requested: `python3` resolves and reports 3.10 or newer from the current process (not a Microsoft Store alias, not Python 2), `oxipng` either resolves or is explicitly reported as deferred, and the Node major version available to the `.mjs` scripts is recorded.
...
- Never install one Creator line's editor extensions into the other line's project, never install both lines side by side, and never claim the MCP extension works on Creator 2.x — that line has no MCP extension at all.
