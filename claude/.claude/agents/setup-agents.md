...
The engine decides four things and nothing else: which preflight runs, which ignore template applies, which editor extensions are installable, and which live phase the checkpoint routes to. Inside Cocos, the **major line** decides one more: Creator 3.x gets the MCP extension and a live phase, Creator 2.x gets neither. Beads, Serena, Better Context, CodeGraph, CocoIndex Code, the Blender MCP, and the video analyzer are engine-neutral and are configured the same way in both.
...
10'. Read and run `setup-cocos-extensions`. It installs the vendored project-local editor extensions — `dev-tools` (the quick-dev menu) and `minigame-pack` (the mini-game subpackage packer) — from this package's own `assets/`, with no network access. Both declare `"editor": ">=3.8.0"` and use the Creator 3.x extension format, so in a **2.x** project report them `not-applicable` and install nothing unless the user explicitly overrides, in which case mark the result `pending-manual-verification`. Never overwrite a locally modified extension: back it up, name the differing files, and ask.
10b'. **Creator 2.x stops here.** There is no editor MCP extension for that line, so skip `setup-cocos-mcp`, skip the editor-open checkpoint, and go straight to the final pass — everything a 2.x project needs is editor-independent. Say so in the report rather than leaving the MCP rows blank.
...
- In a Cocos project: the preflight reported the `creator_version` and `creator_major`, and exactly that line's phases ran — a 2.x project installed no MCP extension, registered no `funplay_cocos` entry, and never waited for an editor; a 3.x project did all three.
- `setup-cocos-extensions` reported one state per vendored extension (`dev-tools`, `minigame-pack`), each verified against its `vendor-manifest.json` hashes after copying, with any locally modified file reported instead of overwritten, and an editor reload named as pending when one is required.
...
- Never install the Creator 3.x editor extensions into a Creator 2.x project without an explicit user override, and never claim they work there.
