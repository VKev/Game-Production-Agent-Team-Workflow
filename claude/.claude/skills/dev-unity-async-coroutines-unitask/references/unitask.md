...
- On Unity 6.3, use Unity's native `Awaitable.EndOfFrameAsync()` or the no-runner UniTask overload when the installed UniTask version provides it. Verify the package API rather than copying the older `MonoBehaviour` runner overload.
...
Unity 6.3 introduced native Awaitable support; Unity 6.3 does not provide it.
