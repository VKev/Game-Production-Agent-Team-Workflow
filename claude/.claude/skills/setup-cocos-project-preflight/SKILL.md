...
description: Inspect and classify a Cocos Creator 3.8 repository, its asset-bundle contract, Git/index state, resumable setup checkpoint, client bundles, and the embedded funplay-cocos-mcp server without mutation or UI automation. Use at the start of setup-agents and every resume in a Cocos project to select Phase A, the editor-open checkpoint, the client-restart checkpoint, Phase B, or completed verification without repeating finished work.
---
...
1. Resolve the intended project root. A Cocos project authors `assets/` and a `package.json` carrying `creator.version`; the inspector refuses anything else rather than guessing.
2. Run:
...
4. Route exactly once:
   - `phase-a`: run editor-independent setup (ignore rules, Beads, Serena, Better Context, CodeGraph, CocoIndex, Blender, and the Cocos MCP extension install).
   - `await-editor-open`: preserve Phase A and ask the user to open the exact project in Cocos Creator, because the MCP server is embedded in the editor and nothing live can be verified while it is closed.
...
| `engine` | `editor_imported_project` (a generated `temp/tsconfig.cocos.json` proves the editor has opened this project at least once) and `editor_reachable`. |
| `cocos_mcp` | The project `funplay-cocos-mcp.config.json` path, the installed extension path/version, resolved host/port, tool profile, whether `execute_javascript` safety checks are on, reachability, and the live tool count from `GET /tools`. |
