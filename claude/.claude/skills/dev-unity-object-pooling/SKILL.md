...
Target Unity 6.3 LTS by default. Inspect `ProjectSettings/ProjectVersion.txt`, package versions, scripting backend, target platform, existing pool code, `AGENTS.md`, and the GDD before choosing APIs or changing architecture.
...
- Addressables-aware pool: use only with explicit asset and handle ownership; also load `dev-unity-assets-addressables`.

Do not use `UnityEngine.Pool` from worker threads or Jobs. It is not thread-safe, and Unity objects must remain on the main thread. Load `dev-unity-jobs-burst-native-collections` for data-parallel native workloads.
...
Read `references/implementation-patterns.md` for Unity 6.3-compatible examples.
...
- `dev-unity-project-context`: inspect Unity version, packages, repository structure, and project conventions.
- `dev-unity-gameplay-architecture`: choose pool ownership, feature boundaries, factories, and dependency direction.
- `dev-unity-csharp-collections-queries`: reuse or pool managed collections and buffers correctly.
- `dev-unity-async-coroutines-unitask`: cancel and reset asynchronous work when items are returned.
- `dev-unity-assets-addressables`: manage pooled Addressable instances and handles.
- `dev-unity-performance-profiling`: perform deeper CPU, memory, GC, and target-device measurement.
- `dev-unity-jobs-burst-native-collections`: handle thread-safe native and data-parallel workloads instead of pooling Unity objects from Jobs.
