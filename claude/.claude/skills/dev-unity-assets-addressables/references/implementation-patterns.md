...
These examples express stable ownership patterns, but API signatures are governed by the exact `com.unity.addressables` version in `Packages/packages-lock.json`. Inspect that package before copying them.
