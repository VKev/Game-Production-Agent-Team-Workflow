...
Use the installed package documentation and package source as the final authority when project behavior differs. These sources are official Unity documentation or Unity's official sample repository. The engine target is Unity 6.3; the Addressables target is the exact package locked by the project, not a hard-coded historical release.

## Unity 6.3 package compatibility and version routing

- Unity 6.3 Addressables package page:
  https://docs.unity3d.com/6000.3/Documentation/Manual/com.unity.addressables.html

The remaining `@1.21` links are retained as researched examples. Before using an API, replace that version segment with the exact installed package version when Unity publishes matching package documentation, or inspect the installed package source.
