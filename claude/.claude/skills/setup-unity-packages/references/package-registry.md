...
| Assistant / official Unity MCP | `com.unity.ai.assistant` | Official Unity Registry verified tarball | Resolve the highest stable or prerelease version compatible with Unity `6000.3.21f1`; verify tarball SHA1/package identity and pin exactly. |
| ProBuilder | `com.unity.probuilder` | Official Unity Registry plus matching Editor catalog | Highest verified stable compatible version, pinned exactly. |
| Visual Effect Graph | `com.unity.visualeffectgraph` | Exact matching Editor package catalog | Pin the Unity 6.3 core-package version (17.3 line); the Editor catalog overrides the public registry `latest` tag. |
| glTFast | `com.unity.cloud.gltfast` | Official Unity Registry plus matching Editor catalog | Highest verified stable compatible version, pinned exactly. |
| Cinemachine | `com.unity.cinemachine` | Official Unity Registry plus matching Editor catalog | Highest verified stable compatible version, pinned exactly; this setup has no old-generation preservation branch. |
| Burst | `com.unity.burst` | Official Unity Registry plus matching Editor catalog | Highest verified stable compatible version satisfying all dependency floors, pinned exactly. |
| Collections | `com.unity.collections` | Official Unity Registry plus matching Editor catalog | Highest verified stable compatible version satisfying all dependency floors, pinned exactly. |
...
## Phase-A evidence object

Before the single manifest transaction, create one JSON object with:

- `unity_version`: exactly `6000.3.21f1`.
- `unity_editor_path`: the exact executable path verified by both resolvers.
- `package_evidence`: an object keyed by every package in the submitted manifest map. Each entry contains the exact `manifest_value`, `verification: "verified"`, its authoritative `source`, and a non-empty `integrity` object. Use registry tarball SHA1 for registry packages, the Editor-catalog manifest SHA256 for catalog-only VFX Graph, the verified tag/commit and tagged package-manifest hash for VContainer, and the Assistant tarball SHA1 from `setup-unity-mcp`.
- `archives`: every registered `.unitypackage` archive with `status: "verified"`, SHA256, path-safety result, staged path/version/fingerprint, and update-check result. Use an empty array only when no registered archives exist.

`apply_unity_63_manifest.py` validates this evidence before touching `Packages/manifest.json` and embeds it in the checkpoint. Missing, mismatched, ambiguous, or unverified evidence blocks the transaction.
...
- Unity `6000.3.21f1` requires compatibility profile `feel-6.0-unity-6000.3-find-objects-by-type` from `scripts/unity_compatibility_patches.json`. The profile is valid only for archive SHA-256 `B422040ECBA5EFD0B6B54F2667D7BA2BBBA30E7511E2F10B6BFD8FCEB7DD4438` and all nine registered source-file hashes. It adds the explicit `FindObjectsSortMode.None` parameter required by Unity 6.3 at ten call sites, retaining `FindObjectsInactive.Include` wherever the source requested inactive objects. Unknown archive, Editor, source, hunk, or result drift is `ambiguous` and must stop before mutation.
...
Each registered FImpossible archive has an exact Unity `6000.3.21f1` compatibility profile. In `Plugins - Shared/Math Helpers/Math/FEngineering.cs`, the Optimizers/Spine source hash is `19B81272...` and the Tail/final Legs source hash is `C44751A4...`; all eight exact `PhysicMaterial` substrings are changed to `PhysicsMaterial`, including the two `PhysicMaterialCombine` enum references, so the final shared-helper result hash is `916659C1...`. The Spine profile repairs both exact obsolete `AnimatorUpdateMode.AnimatePhysics` sites (`3` files / `10` hunks); Tail repairs its one exact site (`2` files / `9` hunks); Legs repairs all three exact obsolete sites (`4` files / `11` hunks), including `Core/LegsA.Initialization.cs` and `LegsAnimator.cs`; Optimizers changes only the shared helper (`1` file / `8` hunks). Each profile requires the exact archive, Unity version, source hashes, prepared result hashes, accepted live-import result hashes, file count, and hunk count. Prepare all four archives with their own profile so every serialized import can compile and Legs establishes the registered final shared baseline.
...
- `scripts/unity_compatibility_patches.json` contains only exact-Editor, exact-archive, exact-source compatibility profiles. This portable setup targets only Unity `6000.3.21f1`; an archive that needs a new source/hash-gated Unity 6.3 patch is `ambiguous` until that profile is explicitly researched, reviewed, and registered.
