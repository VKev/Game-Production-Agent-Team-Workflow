...
if ($verificationRequired) {
    $previousLockHash = $lockHashAfterFirst
    $stableLockHash = $null
    for ($passNumber = 2; $passNumber -le $MaxPasses; $passNumber++) {
        $passes += Invoke-UnityPass -Pass $passNumber -LogPath (Join-Path $logDirectory "unity-package-stabilize-$runId-pass$passNumber.log")
        if (Test-Path -LiteralPath $unityLockPath) {
            throw "Unity left its project lock after stabilization pass ${passNumber}: $unityLockPath"
        }
        $currentLockHash = Get-FileSha256OrNull -Path $lockPath
        if ($previousLockHash -eq $currentLockHash) {
            $stableLockHash = $currentLockHash
            break
        }
        $previousLockHash = $currentLockHash
    }
    if ($null -eq $stableLockHash) {
        throw "The package lock did not stabilize within $MaxPasses clean Unity processes. Do not ask the user to open Unity yet."
    }
    $lockHashAfterFinal = $stableLockHash
}
else {
    $lockHashAfterFinal = $lockHashAfterFirst
}
...
$result = [pscustomobject][ordered]@{
    ...
    package_lock_sha256_before = $lockHashBefore
    package_lock_sha256_after = $lockHashAfterFinal
