---
name: unity-async-coroutines-unitask
...
Do not recommend Unity `Awaitable` APIs to Unity 2022.3. Native `Awaitable` support begins in Unity 2023.1. If the project is newer, compare it with UniTask rather than assuming either one is mandatory.
...
| Unity 2023.1+ library API that should avoid a third-party dependency | Consider Unity `Awaitable` |
| Large measured data-parallel CPU workload | Job System/Burst; invoke `unity-jobs-burst-native-collections` |
...
- Pass a lifetime token from the root operation. In Unity 2022.2+, consider `MonoBehaviour.destroyCancellationToken` and `Application.exitCancellationToken`; create a separate token when cancellation must occur on disable.
...
- On Unity 2022.3, use `UniTask.WaitForEndOfFrame(MonoBehaviour)` for true end-of-frame behavior required by operations such as screen reads; `LastPostLateUpdate` is not equivalent.
...
- Preserve Addressables handle ownership and release rules; invoke `unity-assets-addressables` when handles are involved.
...
- Use `unity-project-context` for repository navigation, Unity/package version discovery, and `AGENTS.md` maintenance.
- Use `unity-gameplay-architecture` for ownership, service, feature-boundary, and dependency decisions.
- Use `unity-assets-addressables` for Addressables handles, release ownership, and content loading architecture.
- Use `unity-performance-profiling` for broader performance investigations.
- Use `unity-jobs-burst-native-collections` for measured data-parallel CPU workloads.
