---
name: unity-assets-addressables
description: Documentation-grounded guidance for choosing, implementing, reviewing, and optimizing Unity runtime asset management with direct references, Resources, and Addressables, with primary focus on com.unity.addressables 1.21 for Unity 2022.3. Use for AssetReference, AsyncOperationHandle, addresses and labels, loading or unloading assets and scenes, InstantiateAsync, handle or reference-count leaks, groups and bundle layout, profiles, remote catalogs, CDN delivery, predownloads, content updates, Analyze, Build Layout Report, migration from Resources or AssetBundles, and Addressables build or CI problems. Inspect actual Unity and package versions before using version-sensitive APIs.
---
...
Do not copy Unity 6 or Addressables 2.x APIs into a Unity 2022.3 project without verifying compatibility.
...
Use the project's established coroutine, callback, Task, or UniTask conventions. For deeper async and cancellation decisions, use the `unity-async-coroutines-unitask` skill.
...
- Use `unity-project-context` for repository navigation and project-wide context.
- Use `unity-gameplay-architecture` for feature boundaries and dependency direction.
- Use `unity-async-coroutines-unitask` for async composition and cancellation architecture.
- Use `unity-object-pooling` when instantiated Addressable prefabs are reused frequently.
- Use `unity-performance-profiling` for broad CPU, GPU, memory, and target-device investigation.
...
- [implementation-patterns.md](references/implementation-patterns.md): Unity 2022.3-oriented C# patterns.
