...
description: Maintain hierarchical project context for Unity repositories through root and folder-local AGENTS.md files. Use when an agent enters, explores, implements, moves, renames, or deletes content in a Unity project; when it must decide whether the repository is new or already documented; or when it must navigate the project through folder maps instead of searching blindly.
---
