---
name: lead-unity-agent-pool
description: Decompose a Unity goal into durable, dependency-safe Beads and manage a bounded reusable pool of Codex worker agents. Use when a TechLead must prioritize parallel Unity work, assign exact worker profiles such as unity-developer or setup-agents, reuse idle workers before spawning new ones, coordinate shared-workspace ownership, and converge implementation through evidence-backed review; do not use to implement product code directly.
---

# Unity Tech Lead Agent Pool
...
2. Read the project-generated `.agents/skills/beads/SKILL.md` before issuing any `bd` command. If it is absent or Beads does not resolve the intended workspace, do not initialize or repair it; report that `setup-agents` must prepare the repository.
...
## Protect the shared Unity workspace

Read [unity-parallel-safety.md](references/unity-parallel-safety.md) before dispatching mutating work.
...
- Never let two workers edit the same file or Unity-serialized asset.
