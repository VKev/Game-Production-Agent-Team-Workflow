...
|---|---|
| Unity 2022.3 | No native Unity `Awaitable`; projects commonly use coroutines, Task adapters, or installed UniTask |
| Unity 2023.1+ | Unity supports awaiting `AsyncOperation`-derived APIs and provides pooled `Awaitable` |
...
Unity 2023.1+ allows direct awaiting of `AsyncOperation`-derived APIs. When using `Awaitable`:
...
For Unity 2022.3 or projects using UniTask, route exact continuation timing, cancellation, and conversion choices through `dev-unity-async-coroutines-unitask`.
