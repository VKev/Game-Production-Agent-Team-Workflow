...
10. Parse `.cocoindex_code/settings.yml`, then merge every entry from [assets/Unity.exclude-patterns.yml](assets/Unity.exclude-patterns.yml) into its `exclude_patterns` list:
   ...
   - Exclude Unity cache/build roots, generated content, serialized scene/asset formats, generated IDE/project files, and only the exact registered third-party roots from the bundled policy. Never broaden those vendor entries to `Assets/Plugins/**`, `Assets/**`, `Packages/**`, `Tests/**`, `Editor/**`, or `ThirdParty/**`; `Assets/Plugins/Game/Probe.cs` must remain eligible.
