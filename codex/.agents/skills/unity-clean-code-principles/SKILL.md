---
name: unity-clean-code-principles
...
7. **Evaluate optimization only after correctness.**
   ...
   - Use `unity-performance-profiling` for nontrivial performance claims.
...
- Use `unity-gameplay-architecture` for cross-system architecture and pattern selection.
- Use `unity-csharp-collections-queries` for collections, loops, LINQ, buffers, and query costs.
- Use `unity-object-pooling` for pool ownership, sizing, and reset contracts.
- Use `unity-async-coroutines-unitask` for asynchronous lifetime, cancellation, and exception ownership.
- Use `unity-assets-addressables` for asset-handle and content ownership.
- Use `unity-performance-profiling` for measured optimization.
- Use `unity-jobs-burst-native-collections` only for justified data-parallel workloads.
