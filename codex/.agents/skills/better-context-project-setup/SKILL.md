---
name: better-context-project-setup
...
2. Require the root `.gitignore` to ignore `.better-context/`, `.ctx-summaries.json`, and Markdown files, and confirm none is tracked. When orchestrated by `setup-agents`, `unity-gitignore-setup` owns these rules and must complete first.
...
Better Context supplies persistent root-to-folder orientation before targeted source retrieval. It does not replace CodeGraph, CocoIndex Code, Serena, direct source reads, compilation, or Unity tests.

Maps remain structural by default. A coding agent may add a concise, verified responsibility summary only when it materially improves future navigation:

```powershell
better-context-unity --root <project-root> agents `
...
Use `--remove-summary <project-relative-path>` to remove a stale or incorrect summary. The CLI persists optional summaries in `.ctx-summaries.json`; never edit generated table rows by hand.
