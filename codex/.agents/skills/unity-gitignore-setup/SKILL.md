---
name: unity-gitignore-setup
description: Add, verify, or repair a Unity project .gitignore from bundled official Unity and project-local AI templates without overwriting existing rules. Use when preparing a copied Codex agent package, when Unity-generated files are not ignored, or when .codex, .agents, .better-context, .ctx-summaries.json, other local AI state, and Markdown files must stay out of Git.
---
