...
// Đo và xoá các thư mục cache của project Cocos Creator 2.x. Không đụng tới
// Editor API để test được bằng node thuần (xem cuối file).
//
// Khác bản 3.x đúng một chỗ: bộ thư mục. Creator 2.x KHÔNG có `profiles/`;
// cấu hình cục bộ của máy nằm hết trong `local/`. `settings/` là cấu hình
// project dùng chung (có trong git) và `packages/` là extension của chính
// project — cả hai không bao giờ được đụng tới.
...
// Chỉ những thư mục này mới được phép xoá. Mọi thứ khác — nhất là assets/,
// settings/ và packages/ — không bao giờ đụng tới.
const CLEARABLE = ['library', 'temp', 'build', 'local'];
...
const TARGETS = {
    ...
    config: {
        ...
        dirs: ['local'],
        restart: true,
        detail: 'Layout Editor, cấu hình preview, preset build cục bộ của MÁY NÀY. settings/ dùng chung thì không đụng.',
    },
};
...
async function emptyTrash(root) {
    ...
    for (const name of names) {
        try {
            await rmrf(path.join(trashRoot, name));
            removed++;
        } catch (e) {
...
/**
 * Xoá đệ quy, chạy được trên Node cũ.
 *
 * Creator 2.4 nhúng Electron/Node cũ hơn nhiều bản 3.8: `fs.promises.rm` chỉ có
 * từ Node 14.14. Dùng thẳng như bản 3.x thì trên Editor 2.x nó `undefined` và
 * thư mục rác không bao giờ được dọn. Ưu tiên API mới khi có, rơi xuống
 * `rmdirSync({recursive:true})` (Node 12+), cuối cùng là tự đệ quy.
 */
async function rmrf(target) {
    ...
    if (st.isDirectory()) {
        try {
            ...
            for (const name of fs.readdirSync(target)) await rmrf(path.join(target, name));
            fs.rmdirSync(target);
            return;
        }
    }
...
module.exports = { TARGETS, TRASH_DIR, CLEARABLE, measure, human, report, trash, clear, emptyTrash, rmrf };

// Chạy trực tiếp để xem báo cáo mà không cần mở Editor:
//   node packages/dev-tools/cache.js
