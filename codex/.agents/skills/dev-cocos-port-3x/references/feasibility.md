...
```bash
python3 <skills-dir>/dev-cocos-port-3x/scripts/triage-scan.py \
...
| `src/settings.json`, `application.js`, `chunks/`, `ENGINE_VERSION="3.x"` | **Cocos Creator 3.x** → đúng nhánh của skill này |
| `window._CCSettings` trong `src/settings*.js`, `ENGINE_VERSION="2.x"`, scene `.fire` | **Cocos Creator 2.x** → dùng `/dev-cocos-port-2x`, quy trình khác hẳn (phải dựng lại project 2.4.x trước) |
...
```bash
# chạy trơn
python3 <skills-dir>/dev-cocos-port-3x/scripts/serve-local.py --root <BUILD> --port 0
...
| 2 | Đủ asset | `manifestClosure` + **map rate** của `extract-cocos3x-assets.py` | thiếu > 0 file, hoặc map rate < 90% → mirror/bóc lại **trước** khi làm gì khác |
...
| 5 | Shader / native riêng | `effects.custom`, `risks.nativeJsb` | > 5 shader riêng — format effect 3.x giữ được, nhưng vẫn phải đối chiếu từng cái |
...
phạm vi không thu về được bản offline · `extract-cocos3x-assets.py` báo map rate
dưới 50% (đọc sai cấu trúc build).
...
- class × hệ số obfuscation — mốc thô: 1 class L1 ≈ 0,5–1 ngày-người khi có AI
  hỗ trợ (agent `cocos-port-class` fan-out song song). Nguồn 3.x thường rẻ hơn nguồn
  2.x cùng quy mô vì không có bước tái dựng trung gian
