---
name: dev-unity-ui-controller-binding
...
5. Use `dev-unity-gameplay-architecture` first when the architectural style itself is still undecided.
...
Read `references/sources.md` when checking uGUI lifecycle, hierarchy discovery, dynamic row lifetime, button behavior, or UI profiling against Unity's official documentation.

## Related skills

- `dev-unity-locale-manager` for language selection, localized bindables, and republishing dynamic screen snapshots after a locale change.
- `dev-unity-gameplay-architecture` for selecting the wider feature architecture and ownership boundaries.
- `dev-unity-clean-code-principles` for naming, small responsibilities, and safe refactors.
- `dev-unity-object-pooling` when `BindList` row churn is measured as a problem.
- `dev-unity-performance-profiling` before adding selective invalidation or recycled scrolling complexity.
