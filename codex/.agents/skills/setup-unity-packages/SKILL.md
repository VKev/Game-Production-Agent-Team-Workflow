...
- **Editor-closed preparation:** require the exact project to be closed. Complete archive inventory/path/hash checks, installed marker and fingerprint inspection, registered mirror checks, independent update confirmation, VContainer release verification, and any allowed temporary candidate download. Read the project's exact Unity version and render-pipeline declarations, resolve every required UPM dependency before mutation, and contribute exact verified specs to the orchestrator's single manifest merge. Generate every required derived archive below `.agent-temp/unity-package-staging/` with `scripts/prepare_unitypackage.py`. After all setup skills contribute their specs, this skill owns the bounded hidden Unity stabilization script that resolves/compiles the complete graph and, when needed, verifies it in a second clean process. Do not import an archive, run post-import setup, or run tests. Mark resolved UPM packages as stable and asset imports as `pending (editor-ready)`.
- **Live pass:** use the one already-open target Editor for archive imports, reloads, and post-import setup. Verify the Phase-A UPM checkpoint before importing anything; do not add or upgrade VContainer, Cinemachine, Burst, Collections, or another manifest-declarable dependency in this process. Preserve vendor import order. Wait for Unity-controlled compilation/reload whenever it blocks the next operation, but do not run a broad console audit or Unity tests after each package. Contribute final compilation, markers, console, and relevant existing-test evidence to the orchestrator's single consolidated pass. A request to close/reopen Unity is an unexpected Phase-A failure, not a normal live step.
...
During `setup-agents`, resolve the complete dependency set while the project is closed, contribute it to the orchestrator's single manifest merge, and stabilize it before the user opens Unity. Never edit `packages-lock.json`. Outside orchestration, a live `Client.AddAndRemove` batch remains allowed when this skill runs standalone.
...
- Resolve Cinemachine, Burst, and Collections with `scripts/resolve_unity_registry_packages.ps1`, using the exact project root and exact matching `Unity.exe`. Require `status=verified-compatible-set` and `cross_package_constraints_verified=true`. The script combines official Unity Registry metadata with the matching Editor's package catalog, rejects prereleases, compares semantic versions numerically, honors the `unityRelease` patch floor, and verifies selected packages satisfy one another's declared version floors; do not substitute a registry-wide latest tag or a version from another project.
- For a missing Cinemachine package, select the highest stable exact-patch-compatible version. For an existing healthy Unity Registry package, pass its installed major through `PreserveMajorJson` and update only within that generation. Never auto-upgrade Cinemachine 2.x to 3.x. Treat an unparsable version, local/embed source, fork, newer installed version, incompatible Editor, unknown source, or unresolved major conflict as `ambiguous`.
...
- Deduplicate by package id against specs contributed by `setup-unity-mcp`. A disagreement is `ambiguous`; do not let last-writer-wins choose the version. Merge the complete validated map once while preserving unrelated manifest entries and style. Never hand-edit the lock file.
- Run `scripts/stabilize_unity_package_graph.ps1` only after the orchestrator completes that single merge. Require the exact matching Unity executable and no interactive Editor/lock file. Reuse its successful checkpoint without launching Unity when the Editor version, manifest hash, lock hash, resolved native graph, and compiled-library marker still match. Otherwise the first hidden process resolves and compiles the whole graph. If the preflight native graph was stale, the lock changed, or the first process reports recognized Burst/native restart evidence, the script launches one verification-only clean process; require identical lock bytes afterward. This process boundary consumes a Burst/native package restart before handoff. An unrecognized nonzero exit, timeout, unstable second lock, package/license failure, or compiler failure that remains on verification blocks the user-open checkpoint.
- In the live pass, compare the resolved package graph and lock hash with the Phase-A checkpoint. Verify package IDs, exact sources/versions, VContainer assemblies, `CinemachineBrain`, generation-appropriate camera type, and Burst/Collections availability. If a manifest-declarable dependency is missing or stale, stop as `Phase A incomplete`; do not submit `Client.Add`, `Client.AddAndRemove`, or ask for a planned interactive restart.
...
- During `setup-agents`, the only Phase-A Unity process is `scripts/stabilize_unity_package_graph.ps1` after the complete manifest merge. Never use it to import archives or test, open a second Editor in the live pass, mutate manifest-declarable UPM packages live, or run repeated broad test passes between package imports.
